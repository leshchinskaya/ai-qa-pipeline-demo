...
const DevicesQR = {
            ...
            <div class="modal-content modal-sm">
                ...
                <div class="modal-body">
                    ...
                    <div id="qr-scanner-timeout-hint" class="qr-scanner-timeout-hint hidden" role="status" aria-live="polite">
                        Не удалось распознать QR — попробуйте загрузить картинку или ввести ID вручную
                    </div>
                    <div class="qr-scanner-fallback">
                        ...
                        <div class="qr-manual-row">
                            <input type="text" id="qr-manual-input" class="form-input" placeholder="…или введите ID устройства вручную" aria-label="ID устройства">
    ...
    renderAvailableState(device) {
        ...
        wrap.innerHTML = `
            ${isShared ? `
            <label class="form-group qr-user-picker">
                <span class="form-label">Выбрать пользователя</span>
                <input type="search" id="qr-user-search" class="form-input" placeholder="Имя или email" autocomplete="off" aria-controls="qr-user-suggest">
                <ul id="qr-user-suggest" class="qr-user-suggest hidden" role="listbox"></ul>
                <small id="qr-user-selected-name" class="form-hint hidden"></small>
            </label>` : ''}
        ...
        // BR.05.30 — поиск пользователя для shared-аккаунта.
        if (isShared) {
            const searchInput = wrap.querySelector('#qr-user-search');
            const suggest = wrap.querySelector('#qr-user-suggest');
            const selectedLabel = wrap.querySelector('#qr-user-selected-name');
            let debounceId = null;
            ...
            const renderSuggestions = (query) => {
                ...
                const filtered = q.length >= 2
                    ...
                    : [];
                if (!filtered.length) {
                    if (q.length >= 2 && usersLoading) {
                        suggest.innerHTML = '<li class="qr-user-suggest-status" aria-disabled="true">Загрузка пользователей…</li>';
                ...
                suggest.innerHTML = filtered.slice(0, 8).map((u) => `
                    <li role="option" data-user-id="${this.escape(u.id)}" tabindex="0">
                        <strong>${this.escape(u.displayName || u.email)}</strong>
                        ${u.email ? `<span> · ${this.escape(u.email)}</span>` : ''}
                    </li>
                `).join('');
                suggest.classList.remove('hidden');
                suggest.querySelectorAll('li[data-user-id]').forEach((li) => {
                    li.addEventListener('click', () => {
                        const id = li.getAttribute('data-user-id');
                        const user = (Users.users || []).find((u) => u.id === id);
                        if (!user) return;
                        this._selectedSharedUserId = id;
                        wrap.dataset.selectedUserId = id;
                        if (selectedLabel) {
                            selectedLabel.textContent = `Выбран: ${user.displayName || user.email}`;
                            selectedLabel.classList.remove('hidden');
                        }
                        searchInput.value = user.displayName || user.email;
                        suggest.classList.add('hidden');
                    });
                });
            };
            searchInput.addEventListener('input', () => {
                ...
                debounceId = setTimeout(() => renderSuggestions(searchInput.value), 150);
            });
        }
    ...
    async submitBooking() {
        ...
        try {
            ...
            const bookingRecord = {
                ...
                source: 'qr',
                // BR.05.18 — для shared-аккаунта фиксируем, кто оформлял бронь.
                bookedBy: isShared ? actorUid : null,
                bookedByName: isShared ? actorName : null,
                currentProjectIds: projectContext?.projectIds || null,
                currentProjectCodes: projectContext?.projectCodes || null,
                currentProjectNames: projectContext?.projectNames || null,
                currentProjectId: projectContext?.projectId || null,
            ...
            if (err && err.code === 'qr/device-busy') {
                ...
                // BR.05.18 / 05.30 — наряду с toast рисуем inline-баннер с описанием
                // ошибки и кнопкой «Повторить» прямо в #qr-booking-body
                // (manual-test component_tests.json: ошибки записи бронирования).
                Toast.show('Не удалось оформить бронь: ' + err.message, 'error');
                this.renderBookingErrorBanner(err && err.message ? err.message : 'Неизвестная ошибка');
            }
            if (confirmBtn) confirmBtn.disabled = false;
        } finally {
            this._bookingInProgress = false;
    ...
    /**
     * BR.05.18 / 05.30 — inline-баннер ошибки бронирования с кнопкой «Повторить».
     * Используется catch-блоком submitBooking() как UI поверх toast-фоллбэка.
     */
    renderBookingErrorBanner(message) {
        const body = document.getElementById('qr-booking-body');
        if (!body) return;
        const existing = body.querySelector('.qr-booking-error');
        if (existing) existing.remove();
        const banner = document.createElement('div');
        banner.className = 'qr-booking-error';
        banner.setAttribute('role', 'alert');
        banner.innerHTML = `
            <div class="qr-booking-error-text">Не удалось оформить бронь: ${this.escape(message)}</div>
            <div class="qr-booking-error-actions">
                <button type="button" class="primary-btn" data-qr-action="retry">Повторить</button>
            </div>
        `;
        const retryBtn = banner.querySelector('[data-qr-action="retry"]');
        if (retryBtn) {
